...
# Db2
# ============================================================================
# This example can be used in two modes:
# 1. Create mode: create a new dataplane and engine with terraform apply
# 2. Import-then-destroy mode: import already existing BYOC resources into
#    Terraform state, then use terraform plan -destroy / terraform destroy
#
# Prerequisites:
# 1. Set bearer token: export TF_VAR_bearer_token="your-token"
# 2. Fill in terraform.tfvars or variables.auto.tfvars with your
#    subscription_id, dataplane_id, engine_name, and Azure details
# 3. Ensure Azure credentials are set in terraform variables
#
# Create mode:
#   terraform init
#   terraform plan
#   terraform apply
#
# Import-then-destroy mode for existing resources:
#   terraform init
#   terraform import ibm_byoc_dataplane.test_azure_dataplane "f038d818-2358-42f0-a84a-c32656597586/ffc234dd-5af0-408c-bcd6-add46b47c86f"
#   terraform import ibm_byoc_engine.test_db2_engine "f038d818-2358-42f0-a84a-c32656597586/ffc234dd-5af0-408c-bcd6-add46b47c86f/<engine-id>"
#   terraform plan -destroy
#   terraform destroy
#
# Exact import ID formats:
# - Dataplane import ID:
#   <subscription_id>/<dataplane_id>
# - Engine import ID:
#   <subscription_id>/<dataplane_id>/<engine_id>
#
# Example delete workflow using the values in this file:
#   terraform init
#   terraform import ibm_byoc_dataplane.test_azure_dataplane "f038d818-2358-42f0-a84a-c32656597586/ffc234dd-5af0-408c-bcd6-add46b47c86f"
#   terraform import ibm_byoc_engine.test_db2_engine "f038d818-2358-42f0-a84a-c32656597586/ffc234dd-5af0-408c-bcd6-add46b47c86f/<replace-with-existing-engine-id>"
#   terraform plan -destroy
#   terraform destroy
#
# Important:
# - If the resources already exist but are not in Terraform state, terraform plan
#   will show + create.
# - To get a delete plan, the existing dataplane and engine must first be
#   imported into Terraform state.
# - The engine import requires the real existing engine ID, not engine_name.
# ============================================================================
...
#   dataplane_id    = var.dataplane_id
#
#   # Enable this only when reading an already existing dataplane.
#   # For create flows, this data source causes plan-time 404 errors before the
#   # dataplane resource is created.
# }
